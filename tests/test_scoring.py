...
from channel_governance.models import MetricRule, PartnerRecord
from channel_governance.scoring import normalize, score_partner


def test_linear_normalization_is_bounded() -> None:
    rule = MetricRule(pillar="x", method="linear", bad=0, good=10, weight=1)
...
def test_optimal_inventory_band_is_not_lower_is_better() -> None:
    rule = MetricRule(pillar="x", method="optimal_band", low=45, high=90, hard_low=0, hard_high=180, weight=1)
...
def test_all_metrics_missing_returns_unrated_not_zero(policies) -> None:
    partner = PartnerRecord(
        ...
        lifecycle_stage="GROWTH", partner_type="DISTRIBUTOR",
    )
