...
| `lifecycle_stage` | enum | yes | `EMERGING`, `GROWTH`, `MATURE`, `MAINTENANCE` |
...
| Pillar | Fields |
|---|---|
| Commercial | `annual_revenue`, `target_achievement_pct`, `yoy_growth_pct` |
| Market | `active_dealers`, `geographic_coverage_pct` |
| Operational | `inventory_days`, `forecast_accuracy_pct` |
| Financial | `payment_on_time_pct`, `ar_overdue_90d_pct` |
| Capability | `certified_engineers`, `training_completion_pct`, `demo_capability` |
| Compliance | `data_reporting_quality_pct`, `pricing_violations`, `unauthorized_sales_incidents` |
